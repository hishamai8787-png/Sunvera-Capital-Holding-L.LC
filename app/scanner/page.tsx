...
export const metadata = { title: "Scanner — Sunvera Analyst" };
            ...
            {flagged.length > 0 && (
              <section>
                <h2 className="text-sm font-semibold text-slate-200 mb-3">
                  🚩 Flagged for investigation ({flagged.length})
                </h2>
                    ...
                    >
                      <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                        <div>
                          ...
                          >
                            {o.companyName}{" "}
                            <span className="text-slate-500 font-normal">({o.symbol})</span>
                          </Link>
                          <span className="text-xs text-slate-500 ml-2">{o.sector}</span>
                        </div>
                        <div className="flex items-center gap-3 text-sm">
                          <span className="tabular-nums text-slate-300">${o.price.toFixed(2)}</span>
                          <span className="text-xs font-semibold rounded-full px-2.5 py-1 bg-amber-400/15 text-amber-300">
                      ...
                      <div className="flex gap-3 text-xs">
                        <Link href={`/analyze/${o.symbol}`} className="text-amber-300/80 hover:text-amber-200">
                          Full analysis →
                        </Link>
                        <Link href={`/credit/${o.symbol}`} className="text-slate-400 hover:text-slate-200">
                          Credit view →
                        </Link>
                      </div>
                    </div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="text-xs uppercase tracking-wider text-slate-500 bg-slate-900">
                        ...
                        <th className="px-3 py-2.5 text-right font-medium">Opportunity</th>
                        <th className="px-4 py-2.5 text-left font-medium">Signals / Risks</th>
                      </tr>
                    </thead>
                    <tbody>
                      {[...flagged, ...rest].map((o) => (
                        <tr key={o.symbol} className={`border-t border-slate-800/60 ${o.flagged ? "bg-amber-400/5" : ""}`}>
                          <td className="px-4 py-2.5">
                            <Link href={`/analyze/${o.symbol}`} className="font-medium text-slate-100 hover:text-amber-300">
                              {o.symbol}
                            </Link>
                            <span className="text-xs text-slate-500 ml-2 hidden md:inline">{o.companyName}</span>
                          </td>
                          ...
                          <td className={`px-3 py-2.5 text-right tabular-nums font-medium ${o.score >= 70 ? "text-emerald-300" : o.score >= 60 ? "text-amber-300" : "text-red-300"}`}>
                            {o.score}
                          </td>
                          <td className="px-3 py-2.5 text-right tabular-nums text-slate-200 font-medium">
                            {o.flagged ? "🚩 " : ""}{o.opportunityScore}
                          </td>
